...
param(
    [Parameter()]
    [string]$Repository = "Chris-Wolfgang/IEnumerable-Extensions",
...
if ($Repository -eq "Chris-Wolfgang/IEnumerable-Extensions" -or -not $Repository) {
    ...
    try {
        ...
        if ($Repository -eq "Chris-Wolfgang/IEnumerable-Extensions") {
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "   ℹ️  Additional checks (not required for merge):" -ForegroundColor Gray
